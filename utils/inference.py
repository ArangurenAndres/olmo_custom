...
class InferenceCallback(Callback):
    ...
    def run_inference(self, step):
        ...
        try:
            ...
            if is_distributed() and get_rank() == 0:
                print(f"[Step {step}] Running inference on rank 0...")
            elif is_distributed():
                print(f"[Step {step}] Skipping inference on rank {get_rank()}")
                return
            else:
                print(f"[Step {step}] Running inference (single GPU)...")
            ...
            # Create input tensor without causing CUDA sync
            input_tensor = torch.tensor([tokens], dtype=torch.long, device=device)
            generated = tokens.copy()

            with torch.no_grad():
                # Single forward pass only (avoid generation loop for now)
                try:
                    print(f"[Step {step}] Running single forward pass...")
                    logits = self.model(input_tensor)
                    print(f"[Step {step}] Forward pass successful, logits shape: {logits.shape}")
                    
                    # Generate just one token to test
                    ...
                    print(f"[Step {step}] Generated (single token): {decoded}")
                    
                except Exception as forward_error:
                    print(f"[Step {step}] Forward pass failed: {forward_error}")
                    raise
